...
var logAccess = (method, path, status, duration, userId) => {
...
// src/event-relay.ts
async function createEventRelay(opts) {
  ...
  async function flush() {
    ...
    try {
      ...
      notifyLogger.error(`Event relay fetch failed for ids ${ids.join(",")}:`, e);
    }
  }
...
function createServer(options = {}) {
  ...
  function emit(channel, payload, meta) {
    const msg = encode(meta?.id !== undefined ? { ev: channel, data: payload, id: meta.id } : { ev: channel, data: payload });
        ...
        (async () => {
          try {
            for await (const row of backfillHandler(pattern, since, {
              ...
              ws.send(encode({ ev: row.type, data: row.payload, id: row.id }));
            }
          } catch (e) {
            serverLogger.warn(`Backfill ${pattern} since ${since} failed: ${e instanceof Error ? e.message : String(e)}`);
          }
        })();
      }
      return;
...
export { createLogger, wsLogger, dbLogger, authLogger, serverLogger, notifyLogger, accessLogger, runtimeLogger, logAccess, logWsAccess, createEventRelay, createServer };
